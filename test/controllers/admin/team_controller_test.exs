defmodule Musiny.Admin.TeamControllerTest do
  use Musiny.ConnCase
  ...
  test "creates resource and redirects when data is valid", %{conn: conn} do
    conn = post conn, admin_team_path(conn, :create), team: @valid_attrs
  ...
  test "renders page not found when id is nonexistent", %{conn: conn} do
    assert_error_sent 404, fn ->
      get conn, admin_team_path(conn, :show, -1)
    end
  end
  ...
  test "updates chosen resource and redirects when data is valid", %{conn: conn} do
    team = Repo.insert! %Team{}
    conn = put conn, admin_team_path(conn, :update, team), team: @valid_attrs
